...
from environ.constants import (
    DATA_PATH as DATA_DIR,
    PROCESSED_DATA_PATH as PROCESSED_DIR,
)
...
def validate_onchain(w3: Web3, req: pd.DataFrame, fin: pd.DataFrame):
    ...
    wq = w3.eth.contract(address=w3.to_checksum_address(WQ_ADDR), abi=WQ_ABI)
    ...
    our_unfin_steth_approx = req[req["request_id"] > our_last_fin]["amount_steth"].sum()
    ...
    ok4 = check(
        ...
        abs(chain_unfin_steth - our_unfin_steth_approx) / max(chain_unfin_steth, 1)
        < 0.01,
...
def validate_internal(req: pd.DataFrame, fin: pd.DataFrame):
    ...
    gaps = (
        fin_s["from_request_id"].iloc[1:].values
        - fin_s["to_request_id"].iloc[:-1].values
        - 1
    )
    has_gaps = (gaps != 0).sum()
    check(
        "Finalization batches contiguous (no gaps/overlaps)",
        has_gaps == 0,
        f"{has_gaps} discontinuities" if has_gaps else "",
    )
    ...
    req_daily = (
        req_ev.groupby("block_number")["ratio"]
        .mean()
        .reset_index()
    ...
    check(
        "stETH/shares ratio non-decreasing (rewards accrue)",
        decreases == 0,
        f"{decreases} large drops (>1%)" if decreases else "",
    )

    # 2f. ETH locked ≈ shares burned × share rate (within 1%)
    fin_check = fin[fin["shares_to_burn"] > 0].copy()
    fin_check["implied_rate"] = (
        fin_check["amount_eth_locked"] / fin_check["shares_to_burn"]
    )
    # share rate should be in [1.0, 1.5] range
    out_of_range = (
        (fin_check["implied_rate"] < 1.0) | (fin_check["implied_rate"] > 1.5)
    ).sum()
    check(
        "ETH-locked / shares-burned ratio in [1.0, 1.5]",
        out_of_range == 0,
        f"{out_of_range} batches out of range" if out_of_range else "",
    )
    ...
    check(
        "Request timestamps roughly non-decreasing",
        ts_violations == 0,
    ...
    check(
        f"Price coverage for queue dates ({merged_price_days}/{total_days} days)",
...
def validate_price(w3: Web3):
    ...
    curve = w3.eth.contract(address=w3.to_checksum_address(CURVE_POOL), abi=CURVE_ABI)
    ...
    print(
        f"  {'DeFiLlama ({})'.format(llama_date):<25} {llama_price:>16.6f}  {llama_disc:>10.4f}%"
    )
    ...
    if not june17.empty:
        ...
        check(
            "June 17 2022 discount in [4%, 8%] (3AC/Celsius crisis)",
            ok_hist,
            f"actual = {disc_june17:.2f}%",
        )
    ...
    check(
        "Post-V2 max discount < 1% (queue ensures near-par trading)",
        ok_post,
        f"max = {max_post:.3f}%",
    )
